...
// This sets up the spacing and place values for x and o
		board.setBoard(arr);
//another initialization- NOTE: THE 2 MIGHT BE COMBINABLE BUT CHECK FIRST
        int game_continue = 1;
        int turn_counter = 0;
int whose_turn = 0;
        int mess_up = 0;
        int mess_upRewrite = 0;
        String choice1 = "dug";
		...
		//input loop
        //
        //ImportantImportantImportantImportantImportantImportantImportantImportantImportantImportant
            while(mess_up==0 || mess_upRewrite!=0){
                ...
                if(mess_up!=0){
                    for(int i=0;i<spots.length;i++){
                        for(int z=0;z<spots.length;z++){
                            if((choice1.equalsIgnoreCase(spots[i][z]))&&(arr[i][z]=='x'||arr[i][z]=='o')){
                                mess_upRewrite=1;
                            }
                        }
                    }
                ...
                if(mess_upRewrite==1){
                    System.out.print("That spot has already been filled. Where would you like to place your ");
                    if(playrNum==2){
                        if(whose_turn == 1){
                            System.out.println("X?");
                        }
